...
export class CivilUserService {
    async registerUser(name: string, email: string, password: string, idNumber: string, isAdmin: boolean = false): Promise<ICivilUser> {
    ...
    async loginUser(email: string, password: string): Promise<{ token: string; isAdmin: boolean; nicNo: string; userId: string; email: string; name: string }>  {
        ...
        const token = jwt.sign({ id: user._id, email: user.email, isAdmin: user.isAdmin }, process.env.JWT_SECRET || "default_secret", {
            expiresIn: "1h",
        });
    ...
    async registerAdminUser(name: string, email: string, password: string, idNumber: string): Promise<ICivilUser> {
        ...
        const existingUser = await CivilUser.findOne({ email });
        if (existingUser) {
    ...
    async getAllUsers(isAdmin?: boolean) {
        try {
            let query = {};
            // If isAdmin is provided (true or false), add it to the query
            if (typeof isAdmin !== 'undefined') {
                query = { isAdmin: isAdmin };
            }
            
            const users = await CivilUser.find(query).select('-password'); // Exclude password from the response
