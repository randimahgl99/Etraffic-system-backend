...
export class CivilUserService {
  ...
  async loginUser(
    email: string,
    password: string
  ): Promise<{ token: string; userType: boolean; nicNo: string }> {
  ...
  async payFine(fineId: string): Promise<object> {
    const fineObjectId = new mongoose.Types.ObjectId(fineId);

    const fine = await policeIssueFine.findById(fineObjectId);
    if (!fine) {
      throw new Error("Fine not found");
    }

    const fineMnagementData = await FineManagement.findById(
      fine.fineManagementId
    );
  ...
  async payFineStatus(
    ...
    if (session.payment_status == "paid") {
      ...
      updatedTransaction = await Transaction.findByIdAndUpdate(transactionId, {
        status: "PAID",
      });
